...
const chatReq = async (req, res) => {
  try {
    ...
    const prompt = "Give 1 word answer"
    const question = "What's palestine's capital?"
    const message = `Prompt: ${prompt}, Question: ${question}?`;
    ...
    const fileName = `response${fs.readdirSync(folderPath).length + 1}.json`;
